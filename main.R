options(warn = -1)
pca.score.fun = function(XFunData, p, M = 6, pve=0.99){
  ...
  if(var(xFunDimTemp)){
    ...
    pcaCombRes = MFPCA(mFData = XFunData, M=M,
                       uniExpansions = rep(list(list(type = "uFPCA")), p))
    Mmax = sum(cumsum(pcaCombRes$values)/sum(pcaCombRes$values) <= pve)+1
    lambda.values[[1]] = pcaCombRes$values
  }
...
fit.fun = function(gflmmPQL.res,
                   family, data, mComb, p,
                   is.dummy){
  
  fixed = gflmmPQL.res$fixed
  random.scalar = gflmmPQL.res$random.scalar
  random.function = gflmmPQL.res$random.function
  
  formula = fixed
  if(!is.null(random.scalar)) 
    formula = update(formula, random.scalar)
  formula = update(formula, random.function)
  
  random.names = c(all.vars(random.scalar), all.vars(random.function))
  random.names = random.names[!(random.names %in% c('subID', '.'))]
  
  if(is.dummy){
    if(length(mComb)>p) mComb = mComb[-1]
    f2.random.formula = as.formula(paste('~.-',
                                         paste('(0 + f2.score', 1:mComb[2],
                                               '| subID)', collapse = '-',sep='')))
    formula = update(formula, f2.random.formula)
    
    random.names = c(all.vars(random.scalar), all.vars(random.function))
    random.names = random.names[!(random.names %in% all.vars(f2.random.formula))]
  }
  
  
  # warm start.
  initial.start = list(theta=rep(1, length(random.names)), 
                       fixef=gflmmPQL.res$fit0$coefficients)
  
  gflmm.res = glmer(formula = formula, data = data,
                    family = family, start= initial.start,
                    control = glmerControl(check.conv.singular = 'ignore',
                                           calc.derivs = FALSE,
                                           nAGQ0initStep = FALSE))
  
  return(list(gflmm.res=gflmm.res, gflm.res = gflmmPQL.res$fit0))
}

construct_beta_fun = function(eigenComb, Mmax, coefs){
  beta_fun = vector('list', length(eigenComb))
  for(i in 1:length(eigenComb)){
    Xtemp = eigenComb[[i]]@X
    if(length(dim(Xtemp)) == 3){
      Xtemp = Xtemp[1:Mmax, , ]
    }else if(length(dim(Xtemp)) == 2){
      Xtemp = Xtemp[1:Mmax, ]
    }
    betaTemp = apply(Xtemp, -1, function(obj){coefs%*%obj})
    dim(betaTemp) = c(1, nObsPoints(eigenComb[[i]]))
    
    beta_fun[[i]] = funData(argvals = eigenComb[[i]]@argvals, X=betaTemp)
  }
  if(length(eigenComb) == 1){
    betaFun = beta_fun[[1]]
  }else{
    betaFun = multiFunData(beta_fun)
  }
  return(betaFun)
}


get.est.coef.fun = function(pcaRes, gflmm.res, nSubs,
                            gflm.res, mComb, p, 
                            is.dummy, is.after=FALSE, 
                            sig.vars = NULL,
                            is.realData=FALSE){
  
  if(length(mComb)<=p & !is.realData){
    stop('Length of mComb should larger than p')
  }else if(is.realData & !is.after){
    p = 1
  }else if(is.after){
    p = length(mComb) - 1
  }
  ### subject-specific model.
  # construct beta(t), delta(t), beta.i(t)
  coef.fix = fixef(gflmm.res)
  coef.fix.len = length(coef.fix)
  beta.Fun = NULL
  delta.Fun = NULL
  
  if(any(all.vars(gflmm.res@call$formula) %in% 'bscore1')){
    coef.fixed.scalar = coef.fix[1:(coef.fix.len-2*mComb[1])]
    coef.fix.beta = coef.fix[(coef.fix.len-2*mComb[1]+1):(coef.fix.len - mComb[1])]
    coef.fix.delta = coef.fix[(coef.fix.len-mComb[1]+1):(coef.fix.len)]
    beta.Fun = construct_beta_fun(eigenComb = pcaRes$pcaCombRes$functions, 
                                  Mmax = mComb[1], coefs = coef.fix.beta)
    delta.Fun = construct_beta_fun(eigenComb = pcaRes$pcaCombRes$functions, 
                                   Mmax = mComb[1], coefs = coef.fix.delta)

  }else{
    coef.fixed.scalar = coef.fix[1:(coef.fix.len - mComb[1])]
    
    coef.fix.beta = coef.fix[(coef.fix.len-mComb[1]+1):coef.fix.len]
    beta.Fun = construct_beta_fun(eigenComb = pcaRes$pcaCombRes$functions, 
                                  Mmax = mComb[1], coefs = coef.fix.beta)
  }
  
  # beta.i(t)
  coef.rand = ranef(gflmm.res)$subID
  coef.rand.len = ncol(coef.rand)
  coef.rand.scalar = coef.rand[, 1:(coef.rand.len - sum(mComb[-c(1, p)]))]
  
  beta.i.Fun = vector('list', nSubs)
  for(i in 1:nSubs){
    beta.i.Fun.temp = NULL
    if(is.dummy){
      if(p==1)
        stop('gflm is suitable, no need to use gflmm')
      if(p==2){
        coef.rand.beta = coef.rand[i, (coef.rand.len - mComb[2] + 1):coef.rand.len]
        beta.i.Fun.temp = c(beta.i.Fun.temp, 
                            construct_beta_fun(eigenComb = pcaRes$pcaSepRes[[1]]$functions, 
                                               Mmax = mComb[2], 
                                               coefs = as.numeric(coef.rand.beta)))
        
        beta.i.Fun[[i]] = construct_beta_fun(eigenComb = pcaRes$pcaSepRes[[1]]$functions,
                                             Mmax = mComb[2],
                                             coefs = as.numeric(coef.rand.beta))
      }else{
        for(j in 1:(p-1)){
          if(j == (p-1)){
            coef.rand.beta = coef.rand[i, (coef.rand.len - mComb[j+1] + 1):coef.rand.len]
          }else{
            coef.rand.beta = coef.rand[i, (coef.rand.len - sum(mComb[-(1:j)]) + 1):(coef.rand.len - sum(mComb[-(1:(j+1))]))]
          }
          beta.i.Fun.temp = c(beta.i.Fun.temp, 
                              construct_beta_fun(eigenComb = pcaRes$pcaSepRes[[j]]$functions, 
                                                 Mmax = mComb[j+1], 
                                                 coefs = as.numeric(coef.rand.beta)))
        }
        beta.i.Fun[[i]] = multiFunData(beta.i.Fun.temp)
      }
    }else{
      for(j in 1:p){
        if(j == p){
        ...
        if(is.after){
          if(is.null(sig.vars)){
            message('No any functional random effect? sig.vars= NULL')
            stop('just use glm() function!!!!')
          }
          beta.i.Fun.temp = c(beta.i.Fun.temp, 
                              construct_beta_fun(eigenComb = pcaRes$pcaSepRes[[sig.vars[j]]]$functions, 
                                                 Mmax = mComb[j+1], 
                                                 coefs = as.numeric(coef.rand.beta)))
        }else{
          beta.i.Fun.temp = c(beta.i.Fun.temp, 
  ...
  ### beta(t), delta(t) for gflm model.
  coef.fix = coef(gflm.res)
  coef.fix.len = length(coef.fix)
  gflm.coef.scalar = coef.fix[1:(coef.fix.len-2*mComb[1])]
  beta.Fun.no = NULL
  delta.Fun.no = NULL
  
  coef.fix.beta = coef.fix[(coef.fix.len-2*mComb[1]+1):(coef.fix.len - mComb[1])]
  beta.Fun.no = construct_beta_fun(eigenComb = pcaRes$pcaCombRes$functions, 
                                   Mmax = mComb[1], coefs = coef.fix.beta)
  coef.fix.delta = coef.fix[(coef.fix.len-mComb[1]+1):(coef.fix.len)]
  delta.Fun.no = construct_beta_fun(eigenComb = pcaRes$pcaCombRes$functions, 
                                    Mmax = mComb[1], coefs = coef.fix.delta)
  
  
  
  return(list(gflmm.coef.fun.res = list(beta.Fun=beta.Fun, delta.Fun=delta.Fun, 
                                        beta.i.Fun=beta.i.Fun,
                                        coef.rand.scalar = coef.rand.scalar,
                                        coef.fixed.scalar = coef.fixed.scalar),
              gflm.coef.fun.res = list(beta.Fun.no=beta.Fun.no, 
                                       delta.Fun.no=delta.Fun.no,
                                       scalar.no = gflm.coef.scalar)))
}
